...
export default function TripDetailsScreen() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'places' | 'expenses' | 'memories' | 'polls'>('overview');

  // Bottom Sheet Drawer State: null | 'place' | 'expense' | 'memory' | 'poll'
  const [activeDrawer, setActiveDrawer] = useState<'place' | 'expense' | 'memory' | 'poll' | null>(null);
  ...
  const [memoryFolders, setMemoryFolders] = useState([
    ...
    },
    {
      ...
      expanded: true,
      photos: [
        { id: '3', caption: 'Resort morning coffee ☕', url: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=600&q=80' },
      ],
    },
  ]);
  ...
  const totalPhotosCount = memoryFolders.reduce((sum, f) => sum + f.photos.length, 0);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.summaryGrid}>
            ...
            <View style={[styles.summaryCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <View style={styles.summaryTextWrapper}>
                <Text style={[styles.summaryValue, { color: colors.text }]}>{totalPhotosCount} Memories</Text>
          ...
          <Text style={[styles.cardTitle, { color: colors.text, marginBottom: 10 }]}>Quick Workspace Actions</Text>
          <View style={styles.quickDrawerGrid}>
            <TouchableOpacity style={styles.drawerTriggerTile} onPress={() => setActiveDrawer('place')} activeOpacity={0.8}>
              <View style={[styles.drawerIconBadge, { backgroundColor: '#00A89615' }]}>
                <Feather name="plus-circle" size={20} color="#00A896" />
              </View>
              <Text style={styles.drawerTriggerLabel}>Add Place</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.drawerTriggerTile} onPress={() => router.push(`/(trips)/calculator?tripId=${trip.id}` as any)} activeOpacity={0.8}>
              <View style={[styles.drawerIconBadge, { backgroundColor: '#F59E0B15' }]}>
                <Text style={styles.rupeeIconMetric}>₹</Text>
              </View>
              <Text style={styles.drawerTriggerLabel}>Add Expense</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.drawerTriggerTile} onPress={() => setActiveDrawer('memory')} activeOpacity={0.8}>
              <View style={[styles.drawerIconBadge, { backgroundColor: '#10B98115' }]}>
                <Feather name="camera" size={20} color="#10B981" />
              </View>
              <Text style={styles.drawerTriggerLabel}>Add Memory</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.drawerTriggerTile} onPress={() => setActiveDrawer('poll')} activeOpacity={0.8}>
              <View style={[styles.drawerIconBadge, { backgroundColor: '#6366F115' }]}>
          ...
          {activeTab === 'overview' && (
            <View style={[styles.workspaceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <Text style={[styles.cardTitle, { color: colors.text }]}>Trip Objective & Details</Text>
              <Text style={[styles.overviewDesc, { color: colors.textSecondary }]}>
                {trip.description || 'Exploring tea estates, waterfalls, scenic views, and hill driving.'}
              </Text>

              <View style={styles.overviewMetaBox}>
                <View style={styles.overviewMetaItem}>
                  <Feather name="navigation" size={16} color="#00A896" style={{ marginRight: 8 }} />
                  <Text style={[styles.overviewMetaText, { color: colors.text }]}>
                    {trip.startingPoint ? `${trip.startingPoint} ➔ ${trip.destination}` : trip.destination}
                  </Text>
                </View>
                ...
                <View style={styles.overviewMetaItem}>
                  ...
                  <Text style={[styles.overviewMetaText, { color: colors.text }]}>{trip.dates || 'Aug 24 - Aug 28, 2026'}</Text>
                </View>
              </View>

              <Text style={[styles.cardTitle, { color: colors.text, marginTop: 16 }]}>Group Members ({trip.members || 5})</Text>
          ...
          {activeTab === 'places' && (
            <View style={[styles.workspaceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <TouchableOpacity style={styles.openDrawerHeaderButton} onPress={() => setActiveDrawer('place')}>
                <Feather name="plus-circle" size={18} color="#FFFFFF" style={{ marginRight: 6 }} />
                <Text style={styles.openDrawerHeaderButtonText}>Add New Place</Text>
              </TouchableOpacity>
          ...
          {activeTab === 'expenses' && (
            <View style={[styles.workspaceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              ...
              <TouchableOpacity style={[styles.openDrawerHeaderButton, { backgroundColor: '#F59E0B' }]} onPress={() => setActiveDrawer('expense')}>
                <Text style={[styles.openDrawerHeaderButtonText, { marginRight: 6 }]}>₹</Text>
                <Text style={styles.openDrawerHeaderButtonText}>Record New Expense</Text>
              </TouchableOpacity>

              <Text style={[styles.cardTitle, { color: colors.text, marginTop: 14, marginBottom: 8 }]}>Expense History</Text>
          ...
          {activeTab === 'memories' && (
            <View style={[styles.workspaceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <TouchableOpacity style={[styles.openDrawerHeaderButton, { backgroundColor: '#10B981' }]} onPress={() => setActiveDrawer('memory')}>
                <Feather name="camera" size={18} color="#FFFFFF" style={{ marginRight: 6 }} />
                <Text style={styles.openDrawerHeaderButtonText}>Upload New Memory</Text>
              </TouchableOpacity>

              <Text style={[styles.cardTitle, { color: colors.text, marginTop: 14, marginBottom: 12 }]}>Trip Photo Folders (Date-Wise)</Text>

              {memoryFolders.map((folder) => (
                <View key={folder.id} style={[styles.driveFolderCard, { backgroundColor: colors.surfaceSubtle, borderColor: colors.border }]}>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <View style={styles.driveFolderInfo}>
                      <Text style={[styles.driveFolderTitle, { color: colors.text }]}>{folder.dateLabel}</Text>
                      <Text style={[styles.driveFolderMeta, { color: colors.textSecondary }]}>
                        {folder.photos.length} {folder.photos.length === 1 ? 'photo' : 'photos'}
                      </Text>
                    </View>
                    <Feather name={folder.expanded ? 'chevron-up' : 'chevron-down'} size={18} color={colors.textMuted} />
                  </TouchableOpacity>

                  {folder.expanded ? (
                    <View style={styles.drivePhotosContainer}>
                      {folder.photos.map((photo) => (
                        <View key={photo.id} style={[styles.crispMemoryTile, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                          <Image source={{ uri: photo.url }} style={styles.crispMemoryImage} resizeMode="cover" />
                          <View style={styles.crispCaptionBox}>
                            <Text style={[styles.crispCaptionText, { color: colors.text }]}>{photo.caption}</Text>
                          </View>
                        </View>
                      ))}
                    </View>
          ...
          {activeTab === 'polls' && (
            <View style={[styles.workspaceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
              <TouchableOpacity style={[styles.openDrawerHeaderButton, { backgroundColor: '#6366F1' }]} onPress={() => setActiveDrawer('poll')}>
                <Feather name="check-square" size={18} color="#FFFFFF" style={{ marginRight: 6 }} />
                <Text style={styles.openDrawerHeaderButtonText}>Create Group Poll</Text>
              </TouchableOpacity>
        ...
        <View style={[styles.bottomWorkspaceNav, { backgroundColor: colors.surface, borderTopColor: colors.border }]}>
          {/* Tab 1: Overview */}
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[styles.bottomNavIconWrapper, activeTab === 'overview' && styles.bottomNavIconActive]}>
              <Feather name="grid" size={20} color={activeTab === 'overview' ? '#00A896' : colors.textMuted} />
            </View>
            <Text style={[styles.bottomNavLabel, activeTab === 'overview' && styles.bottomNavLabelActive]}>Overview</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[styles.bottomNavIconWrapper, activeTab === 'places' && styles.bottomNavIconActive]}>
              <Feather name="map-pin" size={20} color={activeTab === 'places' ? '#00A896' : colors.textMuted} />
              <View style={[styles.badgeCounter, activeTab === 'places' ? styles.badgeActive : styles.badgeInactive]}>
                <Text style={styles.badgeText}>{placesList.length}</Text>
              </View>
            </View>
            <Text style={[styles.bottomNavLabel, activeTab === 'places' && styles.bottomNavLabelActive]}>Places</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[styles.bottomNavIconWrapper, activeTab === 'expenses' && styles.bottomNavIconActive]}>
              <Text style={[styles.bottomRupeeIcon, activeTab === 'expenses' && styles.bottomRupeeIconActive]}>₹</Text>
              <View style={[styles.badgeCounter, activeTab === 'expenses' ? styles.badgeActive : styles.badgeInactive]}>
                <Text style={styles.badgeText}>{expensesList.length}</Text>
              </View>
            </View>
            <Text style={[styles.bottomNavLabel, activeTab === 'expenses' && styles.bottomNavLabelActive]}>Expenses</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[styles.bottomNavIconWrapper, activeTab === 'memories' && styles.bottomNavIconActive]}>
              <Feather name="image" size={20} color={activeTab === 'memories' ? '#00A896' : colors.textMuted} />
              <View style={[styles.badgeCounter, activeTab === 'memories' ? styles.badgeActive : styles.badgeInactive]}>
                <Text style={styles.badgeText}>{totalPhotosCount}</Text>
              </View>
            </View>
            <Text style={[styles.bottomNavLabel, activeTab === 'memories' && styles.bottomNavLabelActive]}>Memories</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <View style={[styles.bottomNavIconWrapper, activeTab === 'polls' && styles.bottomNavIconActive]}>
              <Feather name="check-square" size={20} color={activeTab === 'polls' ? '#00A896' : colors.textMuted} />
              <View style={[styles.badgeCounter, activeTab === 'polls' ? styles.badgeActive : styles.badgeInactive]}>
                <Text style={styles.badgeText}>{pollsList.length}</Text>
              </View>
            </View>
            <Text style={[styles.bottomNavLabel, activeTab === 'polls' && styles.bottomNavLabelActive]}>Polls</Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
      ...
      >
        <KeyboardAvoidingView
          ...
          behavior={Platform.OS === 'ios' ? 'padding' : undefined}
        >
          <TouchableOpacity style={styles.modalBackdrop} onPress={() => setActiveDrawer(null)} activeOpacity={1} />

          {/* Drawer Sheet Body */}
          <View style={[styles.bottomDrawerSheet, { backgroundColor: colors.surface }]}>
            ...
            {activeDrawer === 'place' && (
              <View style={styles.drawerContentContainer}>
                <View style={styles.drawerHeaderRow}>
                  <Text style={[styles.drawerTitle, { color: colors.text }]}>Add New Itinerary Place</Text>
                  <TouchableOpacity onPress={() => setActiveDrawer(null)}>
                    <Feather name="x" size={20} color={colors.textMuted} />
                  </TouchableOpacity>
                </View>

                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Place Name</Text>
                <TextInput
                  style={[styles.drawerInput, { backgroundColor: colors.surfaceSubtle, color: colors.text, borderColor: colors.border }]}
                ...
                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Category & Time Slot</Text>
                <View style={styles.drawerRowTwo}>
                  <TextInput
                    style={[styles.drawerInput, { flex: 1, backgroundColor: colors.surfaceSubtle, color: colors.text, borderColor: colors.border }]}
                ...
                <TouchableOpacity style={styles.drawerSubmitButton} onPress={handleSavePlaceDrawer} activeOpacity={0.85}>
                  <Feather name="check" size={16} color="#FFFFFF" style={{ marginRight: 6 }} />
                  <Text style={styles.drawerSubmitText}>Save Place to Trip</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {activeDrawer === 'expense' && (
              <View style={styles.drawerContentContainer}>
                <View style={styles.drawerHeaderRow}>
                  <Text style={[styles.drawerTitle, { color: colors.text }]}>Record Shared Expense</Text>
                  <TouchableOpacity onPress={() => setActiveDrawer(null)}>
                    <Feather name="x" size={20} color={colors.textMuted} />
                  </TouchableOpacity>
                </View>
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <TouchableOpacity
                      style={[styles.drawerInput, styles.dropdownSelectorDrawer, { backgroundColor: colors.surfaceSubtle, borderColor: colors.border }]}
                      onPress={() => setIsPaidByDropdownOpen(!isPaidByDropdownOpen)}
                    >
                      <Text style={[styles.dropdownValueText, { color: colors.text }]} numberOfLines={1}>
                        {expensePaidByInput}
                      </Text>
                      <Feather name={isPaidByDropdownOpen ? 'chevron-up' : 'chevron-down'} size={16} color={colors.textMuted} />
                    </TouchableOpacity>

                    {isPaidByDropdownOpen ? (
                      <View style={[styles.dropdownListContainerDrawer, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                        {memberOptions.map((opt) => (
                          <TouchableOpacity
                            key={opt}
                            style={[styles.dropdownOptionRow, expensePaidByInput === opt && styles.dropdownOptionSelected]}
                            onPress={() => {
                              setExpensePaidByInput(opt);
                              setIsPaidByDropdownOpen(false);
                            }}
                          >
                            <Text style={[styles.dropdownOptionText, { color: colors.text }, expensePaidByInput === opt && styles.dropdownOptionTextSelected]}>
                              {opt}
                            </Text>
                          </TouchableOpacity>
                        ))}
                ...
                <TouchableOpacity style={[styles.drawerSubmitButton, { backgroundColor: '#F59E0B' }]} onPress={handleSaveExpenseDrawer} activeOpacity={0.85}>
                  <Text style={[styles.drawerSubmitText, { marginRight: 4 }]}>₹</Text>
                  <Text style={styles.drawerSubmitText}>Save & Split Expense</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {activeDrawer === 'memory' && (
              <View style={styles.drawerContentContainer}>
                <View style={styles.drawerHeaderRow}>
                  <Text style={[styles.drawerTitle, { color: colors.text }]}>Upload Trip Photo Memory</Text>
                  <TouchableOpacity onPress={() => setActiveDrawer(null)}>
                    <Feather name="x" size={20} color={colors.textMuted} />
                  </TouchableOpacity>
                </View>

                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Photo Caption</Text>
                <TextInput
                  style={[styles.drawerInput, { backgroundColor: colors.surfaceSubtle, color: colors.text, borderColor: colors.border }]}
                ...
                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Target Date Folder</Text>
                <View style={styles.folderSelectRow}>
                  {memoryFolders.map((f) => (
                    <TouchableOpacity
                      key={f.id}
                      style={[styles.folderPillOption, selectedFolderId === f.id && styles.folderPillOptionSelected]}
                      onPress={() => setSelectedFolderId(f.id)}
                    >
                      <Feather name="folder" size={14} color={selectedFolderId === f.id ? '#FFFFFF' : '#00A896'} style={{ marginRight: 4 }} />
                      <Text style={[styles.folderPillText, selectedFolderId === f.id && styles.folderPillTextSelected]}>
                        {f.dateLabel.split('-')[0]}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <TouchableOpacity style={[styles.drawerSubmitButton, { backgroundColor: '#10B981' }]} onPress={handleSaveMemoryDrawer} activeOpacity={0.85}>
                  <Feather name="camera" size={16} color="#FFFFFF" style={{ marginRight: 6 }} />
                  <Text style={styles.drawerSubmitText}>Save Memory to Trip</Text>
                </TouchableOpacity>
              </View>
            )}
            ...
            {activeDrawer === 'poll' && (
              <View style={styles.drawerContentContainer}>
                <View style={styles.drawerHeaderRow}>
                  <Text style={[styles.drawerTitle, { color: colors.text }]}>Create Group Poll</Text>
                  <TouchableOpacity onPress={() => setActiveDrawer(null)}>
                    <Feather name="x" size={20} color={colors.textMuted} />
                  </TouchableOpacity>
                </View>
                ...
                <Text style={[styles.inputLabel, { color: colors.textSecondary }]}>Voting Options</Text>
                <TextInput
                  style={[styles.drawerInput, { backgroundColor: colors.surfaceSubtle, color: colors.text, borderColor: colors.border, marginBottom: 8 }]}
                ...
                <TouchableOpacity style={[styles.drawerSubmitButton, { backgroundColor: '#6366F1' }]} onPress={handleSavePollDrawer} activeOpacity={0.85}>
                  <Feather name="check-square" size={16} color="#FFFFFF" style={{ marginRight: 6 }} />
...
const styles = StyleSheet.create({
  ...
  openDrawerHeaderButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' },
  ...
  pollCardInner: { padding: 12, backgroundColor: '#F8FAFC', borderRadius: 14, marginBottom: 10 },
