...
import { getTripsFromStorage, deleteTripFromStorage, TripData } from '@/utils/storage';
...
export default function TripsTabScreen() {
  ...
  const [selectedTripToDelete, setSelectedTripToDelete] = useState<TripData | null>(null);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          style={[styles.friendsIconButton, { backgroundColor: colors.surfaceSubtle }]}
            ...
            >
              <View style={styles.cardHeader}>
                <View style={{ flex: 1, marginRight: 8 }}>
                  ...
                  <View style={styles.locationRow}>
                    <Feather name="navigation" size={14} color="#00A896" style={{ marginRight: 6 }} />
                    <Text style={[styles.locationText, { color: colors.textSecondary }]}>
                      {trip.startingPoint ? `${trip.startingPoint} ➔ ${trip.destination}` : trip.destination}
                    </Text>
                  </View>
                </View>
                <View style={[styles.statusBadge, { backgroundColor: (trip.statusColor || '#00A896') + '20' }]}>
                  <Text style={[styles.statusText, { color: trip.statusColor || '#00A896' }]}>{trip.status}</Text>
                </View>
              </View>
              ...
              <View style={styles.cardFooter}>
                <View style={styles.footerInfo}>
                  ...
                  <Text style={[styles.footerText, { color: colors.textSecondary }]}>{trip.dates || 'Upcoming'}</Text>
                </View>
                <View style={styles.footerInfo}>
                  <Feather name="users" size={14} color={colors.textMuted} style={{ marginRight: 4 }} />
                  <Text style={[styles.footerText, { color: colors.textSecondary }]}>{trip.members || 1} member</Text>
                </View>
              </View>
            </TouchableOpacity>
          ))
      ...
      <Modal visible={isDeleteDrawerOpen} transparent animationType="none" onRequestClose={closeDeleteDrawer}>
        <View style={styles.modalOverlay}>
          <TouchableOpacity style={styles.backdropTouch} onPress={closeDeleteDrawer} activeOpacity={1} />

          <Animated.View style={[styles.drawerContainer, { transform: [{ translateY: deleteSlideAnim }] }]}>
            ...
            <View style={styles.deleteActionsRow}>
              <TouchableOpacity style={styles.cancelDeleteButton} onPress={closeDeleteDrawer} activeOpacity={0.8}>
                <Text style={styles.cancelDeleteText}>Cancel</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.confirmDeleteButton} onPress={confirmDeleteTrip} activeOpacity={0.85}>
